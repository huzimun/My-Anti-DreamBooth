...
class AdaEA_Base:
    ...
    def agm(self, ori_data, cur_adv, grad, label):
        ...
        loss_self = [loss_func(self.models[idx](adv_exp[idx]), label) for idx in range(self.num_models)] # 自己和自己的下一步损失
        ...
        for j in range(self.num_models):
            for i in range(self.num_models):
                if i == j:
                    continue
                w[j] += loss_func(self.models[i](adv_exp[j]), label) / loss_self[i] * self.beta # 自己和别人的下一步损失

...
def compute_idx_embedding(path, model_name):
    """
    Compute the embedding of each person given images
    """
    ave_embedding = 0
    ...
    for file in os.listdir(path):
        if file.endswith(".jpg") or file.endswith(".png"):
            try:
                count_file += 1
                embedding_objs = DeepFace.represent(img_path = os.path.join(path, file), model_name=model_name, detector_backend="retinaface", align=True)
                embedding = embedding_objs[0]["embedding"]
                embedding = np.array(embedding)
                ave_embedding += embedding
            except:

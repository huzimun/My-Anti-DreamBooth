...
def main(args):
    ...
    for person_id in sorted(os.listdir(args.input_dir)):
        ...
        print(person_dir)
        original_images = load_data(person_dir, args.resolution, 2) # 4张图像，torch.tensor
        
        perturbed_images = original_images.detach().clone()
        ...
        perturbed_images = perturbed_images.to(args.device, dtype=torch.float16) # torch.Size([4, 3, 224, 224])
        clip_image_embeds = image_encoder(perturbed_images, output_hidden_states=True).hidden_states[-2] # torch.Size([4, 257, 1280])
        ...
        uncond_clip_image_embeds = image_encoder( # torch.Size([4, 257, 1280])
            torch.zeros_like(original_images), output_hidden_states=True
        ).hidden_states[-2]
        ...
        for i, t in enumerate(timesteps):
            ...
            latent_model_input = scheduler.scale_model_input(latent_model_input, t) # torch.Size([32, 4, 64, 64])
            pdb.set_trace()
            # predict the noise residual
            noise_pred = unet( # torch.Size([4, 4, 64, 64])
                latent_model_input, # torch.Size([32, 4, 64, 64]), 没有negative prompt的时候是torch.Size([16, 4, 64, 64])，num_samples=1的时候是torch.Size([4, 4, 64, 64])
                t,
            ...
            loss = F.mse_loss(noise_pred.float(), noises.float(), reduction="mean")
            print(f"loss: {loss.detach().item()}")
            # loss.backward()
            # alpha = args.pgd_alpha
            # eps = args.pgd_eps

            adv_images = perturbed_images + args.alpha * perturbed_images.grad.sign()
            eta = torch.clamp(adv_images - original_images, min=-args.eps, max=+args.eps)
        ...
        if args.output_dir :
            ...
            save_image(perturbed_images, save_path)
        pdb.set_trace()
    pdb.set_trace()

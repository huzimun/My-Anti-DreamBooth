export EXPERIMENT_NAME="TED_conda-photomaker"
...
if [ "$DATASET" = "VGGFace2" ]; then
    ...
    EXPERIMENT_NAME=${EXPERIMENT_NAME}"_"${DATASET}
elif [ "$DATASET" = "new-CelebA-HQ" ]; then
    export DATASET_DIR="/data1/humw/Datasets/new-CelebA-HQ"
    EXPERIMENT_NAME=${EXPERIMENT_NAME}"_"${DATASET}
elif [ "$DATASET" = "test" ]; then
    export DATASET_DIR="/data1/humw/Codes/My-Anti-DreamBooth/data/test"
    EXPERIMENT_NAME=${EXPERIMENT_NAME}"_"${DATASET}
elif [ "$DATASET" = "test2" ]; then
    export DATASET_DIR="/data1/humw/Codes/My-Anti-DreamBooth/data/test2"
    EXPERIMENT_NAME=${EXPERIMENT_NAME}"_"${DATASET}
else
...
if [ "$target" = "mist" ]; then
    ...
    export target_image_path="/data1/humw/Codes/FaceOff/target_images/gray"
elif [ "$target" = "face" ]; then
    export target_image_path="/data1/humw/Datasets/VGGFace2"
    export id_map_path="/data1/humw/Codes/FaceOff/max_clip_cosine_distance_map.json"
...
elif [ "$target" = "small_dianwei" ]; then
    export target_image_path="/data1/humw/Codes/My-Anti-DreamBooth/mask_targets/small_dianwei"
elif [ "$target" = "small_yingbu" ]; then
    export target_image_path="/data1/humw/Codes/My-Anti-DreamBooth/mask_targets/small_yingbu"
...
# use AGM
export agm=2 # 1使用AGM，0取平均，2手动调整10:1:1，3手动调整5：1：1，4手动调整100：1：1
if [ "$agm" = 1 ]; then
    ...
    EXPERIMENT_NAME=${EXPERIMENT_NAME}"_agm-10011"
elif [ "$agm" = 111 ]; then
    EXPERIMENT_NAME=${EXPERIMENT_NAME}"_agm-111"
else
    export "invalid agm"
    exit 1
fi
